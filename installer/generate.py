...
def generate_systemd():
    ...
    for dist in distro_systemd_schemas_gateway:
        ...
        distro_systemd_schemas_gateway[dist]['Service']['ExecStart'] = \
                f"+{path_venv}/bin/python3 {path_main} --log=INFO --module=gateway"
    ...
    for dist in distro_systemd_schemas_cluster:
        ...
        distro_systemd_schemas_cluster[dist]['Service']['ExecStart'] = \
                f"+{path_venv}/bin/python3 {path_main} --log=INFO --module=cluster"
...
def generate_deps():
    ...
    def rabbitmq():
        path_rabbitmq_versions_lock =os.path.join(
                os.path.dirname(__file__), '../third-party/rabbitmq', 'version.lock')
        path_rabbitmq_init_script =os.path.join(
                os.path.dirname(__file__), '../third-party/rabbitmq', 'init.sh')
...
if __name__ == "__main__":
    ...
    path_rabbitmq=os.path.join(
            os.path.dirname(__file__), '../third-party/rabbitmq', '')

    path_rabbitmq_builds=os.path.join(
            os.path.dirname(__file__), '../third-party/rabbitmq/builds', '')
